    ...
    fn process_output(
        ...
        match process_perf_stat_output(output) {
            Ok(stats) => {
                ...
                match data.run_kind {
                    RunKind::Clean => {
                        self.clean_stats.push((stats, self_profile));
                    }
                    RunKind::Nll => {
                        self.nll_stats.push((stats, self_profile));
                    }
                    RunKind::BaseIncr => {
                        self.base_incr_stats.push((stats, self_profile));
                    }
                    RunKind::CleanIncr => {
                        self.clean_incr_stats.push((stats, self_profile));
                    }
                    RunKind::PatchedIncrs => {
                        ...
                        self.patched_incr_stats
                            .push((patch.clone(), vec![(stats, self_profile)]));
                    }
                }
                Ok(Retry::No)
            }
